...
st.markdown("""
    ...
    [data-testid="stSidebar"] { background-color: #0b0e11 !important; border-right: 1px solid rgba(255, 255, 255, 0.05); z-index: 1000 !important; }

    /* Horizontal Nav Rail */
    [data-testid="stSidebar"] div[role="radiogroup"] { flex-direction: row !important; justify-content: flex-start !important; gap: 8px !important; flex-wrap: nowrap !important; }
    [data-testid="stSidebar"] div[role="radiogroup"] label {
        background: rgba(255, 255, 255, 0.02) !important; border: 1px solid rgba(255, 255, 255, 0.1) !important;
        border-radius: 8px !important; padding: 10px 8px !important; min-width: 80px !important; position: relative; transition: 0.3s;
    }
    [data-testid="stSidebar"] div[role="radiogroup"] label:has(input:checked) {
        border: 2px solid #ff7a00 !important; background: rgba(255, 122, 0, 0.1) !important; box-shadow: 0 0 15px rgba(255, 122, 0, 0.4);
    }
    [data-testid="stSidebar"] div[role="radiogroup"] label:has(input:checked)::after {
        content: '⌄'; position: absolute; bottom: -25px; left: 50%; transform: translateX(-50%); color: #ff7a00; font-weight: 900; font-size: 1.4rem;
    }
    [data-testid="stSidebar"] div[role="radiogroup"] label div:first-child { display: none !important; }
    [data-testid="stSidebar"] div[role="radiogroup"] label p { font-size: 0.65rem !important; font-weight: 800 !important; margin: 0 !important; color: white !important; text-align: center; }

    .feature-card { background: rgba(255, 255, 255, 0.02); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 24px; padding: 25px; margin-bottom: 20px; box-shadow: 0 4px 24px 0 rgba(0, 0, 0, 0.4); }
    .dev-credit { position: fixed; bottom: 15px; left: 20px; font-size: 0.7rem; font-weight: 800; color: rgba(255, 255, 255, 0.3); letter-spacing: 1px; text-transform: uppercase; }
    .stApp header { visibility: visible !important; background: transparent !important; }
    </style>
""", unsafe_allow_html=True)

# --- 4. PERMANENT ENGINE LOAD (Integrated with train_model.py path) ---
MODEL_PATH = "data/placement_model.pkl"
...
if page_selection == "INGESTION":
    ...
    if 'current_target' in st.session_state:
        ...
        if not jobs.empty:
            for idx, row in jobs.iterrows():
                st.markdown(f'<div class="feature-card"><h4 style="color: #ff7a00; margin: 0;">{row["Title"]}</h4><p style="opacity: 0.8; margin-bottom: 15px;">{row["Company"]}</p><a href="{row["URL"]}" target="_blank" style="background: #ff7a00; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 800; font-size: 0.8rem;">APPLY NOW →</a></div>', unsafe_allow_html=True)